...
describe("BananaSwapPair", () => {
  ...
  async function fixture([wallet, other]: Wallet[], provider: MockProvider) {
    ...
    const banana = await TokenBFactory.deploy();
    await banana.initialize("Banana", "Banana");

    const TokenManager = await ethers.getContractFactory("TokenManager");
    const tokenManager = await TokenManager.deploy();
    await tokenManager.initialize(banana.address, usdt.address);

    let pairBtoken0 = banana.address > usdt.address ? usdt : banana; // token 0 is TokenB
    let pairBtoken1 = banana.address > usdt.address ? banana : usdt;
    ...
    await pairB.initialize(pairBtoken0.address, pairBtoken1.address, banana.address, factory.address, wallet.address);
    await usdtFeeHandle.initialize(usdt.address, banana.address, pairB.address);
  ...
  swapTestCases.forEach((swapTestCase, i) => {
    it(`getInputPrice:${i}`, async () => {
      ...
      await expect(
        pair.swap(0, expectedOutputAmount.add(1), wallet.address, "0x", wallet.address)
      ).to.be.revertedWith("BananaSwap: K");
      await pair.swap(0, expectedOutputAmount, wallet.address, "0x", wallet.address);
    });
  });
  ...
  optimisticTestCases.forEach((optimisticTestCase, i) => {
    it(`optimistic:${i}`, async () => {
      ...
      await expect(
        pair.swap(outputAmount.add(1), 0, wallet.address, "0x", wallet.address)
      ).to.be.revertedWith("BananaSwap: K");
      await pair.swap(outputAmount, 0, wallet.address, "0x", wallet.address);
    });
  });
  ...
  it("swap:token0", async () => {
    ...
    await token0.transfer(pair.address, swapAmount);
    await expect(pair.swap(0, expectedOutputAmount, wallet.address, "0x", wallet.address))
  ...
  it("swap:token1", async () => {
    ...
    await expect(pair.swap(expectedOutputAmount, 0, wallet.address, "0x", wallet.address))
  ...
  it("swap:gas", async () => {
    ...
    const tx = await pair.swap(expectedOutputAmount, 0, wallet.address, "0x", wallet.address);
  ...
  it("price{0,1}CumulativeLast", async () => {
    ...
    await pair.swap(0, expandTo18Decimals(1), wallet.address, "0x", wallet.address); // make the price nice
  ...
  it("feeTo:off", async () => {
    ...
    await token1.transfer(pair.address, swapAmount);
    await pair.swap(expectedOutputAmount, 0, wallet.address, "0x", wallet.address);
  ...
  it("feeTo:on", async () => {
    ...
    expect(await pair.totalSupply()).to.eq(
      MINIMUM_LIQUIDITY.add("749251872006222")
    );
    ...
    expect(await token0.balanceOf(pair.address)).to.eq(
      BigNumber.from(1000).add("748505051092335")
    );
    expect(await token1.balanceOf(pair.address)).to.eq(
      BigNumber.from(1000).add("750000561938905")
    );
  });
});
